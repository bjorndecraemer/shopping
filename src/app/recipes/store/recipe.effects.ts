...
export class RecipeEffects{
  ...
  recipeFetch = this.actions$.ofType(RecipeActions.FETCH_RECIPES).pipe(
    switchMap((action : RecipeActions.FetchRecipes) => {
      return this.httpClient.get<Recipe[]>('https://ng-recipe-book-436f3.firebaseio.com/recipes.json', {
        observe: 'body',
        responseType: 'json'
      })
    }),
  ...
  recipeStore = this.actions$
    .ofType(RecipeActions.STORE_RECIPES).pipe(
      ...
      switchMap(([action, state]) => {
        const req = new HttpRequest('PUT', 'https://ng-recipe-book-436f3.firebaseio.com/recipes.json', state.recipes, {reportProgress: true});
